...
\begin{itemize}
    \item \textbf{s-process} (slow neutron capture): Occurs in asymptotic giant branch (AGB) stars, where neutrons are captured at a rate slower than the beta decay of unstable nuclei.
    \item \textbf{r-process} (rapid neutron capture): Occurs in explosive environments such as supernovae and neutron star mergers, where neutrons are captured at a rate much faster than the beta decay of unstable nuclei.   
    \item \textbf{p-process} (proton capture): Occurs in supernovae, where proton-rich isotopes are produced through proton capture and photodisintegration reactions.
\end{itemize}
\end{theory}
...
\begin{equation}
E_0 = \left(\frac{1}{4} E_G (k_BT)^2\right)^{1/3} = \left(\frac{E_G}{4}\right)^{1/3} (k_BT)^{2/3}.
\end{equation}
